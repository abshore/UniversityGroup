...
//        if ($form->isValid()) {
    ...
    public function ajouterSeanceAction() {
        ...
        $AffectationPlan = $em->getRepository('GestionEmploisBundle:AffectationPlan')->findOneBy(array('classe' => $classe, 'matiere' => $matiere));
...
//        $cl = $request->get('classe');
        ...
        $dates[]=date("l d/m/Y", mktime(0, 0, 0, $m, $j-$w+1, $year) + $i * (3600 * 24));
        }
        $seances=$em->getRepository('GestionEmploisBundle:Emploi')->findBy(array('classe'=>$classe));
        ...
        //$td="";
        for ($i = 0; $i < count($date); $i++) {
            //echo $date[$i]->format('l');
            $joursAct[]= $date[$i]->format('l');
            $dateAct[]= $date[$i]->format('l d/m/Y');
            //$td .="<td>".$date[$i]->format('l')."</td>";
        }
        $un=  array_unique($joursAct);
        //exit;
        
        return $this->render('GestionEmploisBundle:Emploi:EmploiClasse.html.twig', array(
            'classe'=>$classe,
            'year'=>$year,
            'jours'=>$weekdays,
            'seances'=>$seances,
